...
{
    public class AuthenticationHttpMessageHandler<T> : DelegatingHandler where T : class
    {
        ...
        private async Task<Login> LoginAsync()
        {
            var ok = await Task.Run(() =>
            {
                string _rolCliente, _usuarioCliente;
                // caso de llamadas no http
                if (_httpContextAccessor.HttpContext == null)
                {
                    return new Login() { Rol = _jwtSiagieMsaConfiguration.Agp, User = _jwtSiagieMsaConfiguration.Agl };
                }

                _usuarioCliente = _httpContextAccessor.HttpContext.Items["ACTU"] != null ? _httpContextAccessor.HttpContext.Items["ACTU"].ToString() : _jwtSiagieMsaConfiguration.Agl;
                if (_usuarioCliente == _jwtSiagieMsaConfiguration.Agl)
                {
                    _rolCliente = _jwtSiagieMsaConfiguration.Agp;
                }
                else
                {
                    _rolCliente = _httpContextAccessor.HttpContext.Items["Role"] != null ? _httpContextAccessor.HttpContext.Items["Role"].ToString() : _jwtSiagieMsaConfiguration.Agp;
                }
                //  var _rolCliente = _httpContextAccessor.HttpContext.Items["Role"] != null ? _httpContextAccessor.HttpContext.Items["Role"].ToString() : _jwtSiagieMsaConfiguration.Agp;

                return new Login() { Rol = _rolCliente, User = _usuarioCliente };
            });
            return ok;
        }
    }
